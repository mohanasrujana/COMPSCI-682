...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        wtn = 'W' + str(self.num_layers) 
        bn = 'b' + str(self.num_layers)

        
        loss, dx = softmax_loss(scores, y)
        dout, grads[wtn], grads[bn] = affine_backward(dx, affine_cache.pop())
        loss += 0.5 * self.reg * np.sum(np.square(self.params[wtn]))
        grads[wtn] += self.reg * self.params[wtn]
        i = self.num_layers - 2
        ...
        while i >= 0:
          ...
          if self.normalization=='batchnorm':
            dout, grads['gamma' + str(i+1)], grads['beta' + str(i+1)] = batchnorm_backward(dout, bn_cache.pop())
          if self.normalization=='layernorm':
            dout, grads['gamma' + str(i+1)], grads['beta' + str(i+1)] = layernorm_backward(dout, ln_cache.pop())
          
          wtn= 'W' + str(i+1) 
          bn = 'b' + str(i+1)
          dout, grads[wtn], grads[bn] = affine_backward(dout, affine_cache.pop())
